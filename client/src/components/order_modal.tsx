/**
 * Interactive Order Detail Modal — customer submission + payment toggle.
 */
...
export default function OrderModal({
  ...
  const fulfilLabel =
    order.fulfillmentType === 'delivery' ? 'Home Pickup / Delivery' : 'Drop-off / Pickup';
      ...
      >
        <div className="mb-6 flex items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <PaymentBadge order={order} onUpdated={onOrderUpdate} size="md" />
        ...
        <dl className="space-y-4 text-sm">
          ...
          />
          <Row label="Service Type" value={order.serviceType.replace(/-/g, ' ')} />
          <Row label="Estimated Weight" value={`${order.weightKg} kg`} />
          <Row label="Logistics Slot" value={formatDateTime(order.scheduledDate, order.scheduledTime)} />
          <Row label="Fulfilment" value={fulfilLabel} />
          <Row label="Special Instructions" value={order.specialNotes ?? 'None'} />
          <Row label="Billing Total" value={`₱${order.totalFee.toFixed(2)}`} highlight />
        </dl>
...
function Row({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="border-b border-slate-100 pb-3">
      ...
      <dd className={`mt-1 font-medium ${highlight ? 'text-lg text-brand-700' : 'text-slate-800'}`}>
        {value}
      </dd>
    </div>
  );
