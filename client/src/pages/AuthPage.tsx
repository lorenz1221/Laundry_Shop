...
import RecaptchaWidget, { resetRecaptchaWidget } from '../components/RecaptchaWidget';
...
export default function AuthPage() {
  ...
  const [regCaptcha, setRegCaptcha] = useState<string | null>(null);
  const captcha = useRecaptchaConfig();
  ...
  const handleSignIn = async (e: FormEvent) => {
    ...
    if (captcha.enabled && !signInCaptcha) {
      flashWarning('Please complete the CAPTCHA verification.');
      return;
    }
    setLoading(true);
    try {
      await login({
        ...
        recaptchaToken: signInCaptcha ?? undefined,
      });
  ...
  const handleRegister = async (e: FormEvent) => {
    ...
    try {
      await register({
        ...
        role: regRole,
        recaptchaToken: regCaptcha ?? undefined,
      });
          ...
          <div className="rounded-2xl border border-slate-200/80 bg-white/90 p-8 shadow-xl shadow-slate-200/50 backdrop-blur-sm">
            ...
            {tab === 'signin' ? (
              <form onSubmit={handleSignIn} className="space-y-5">
                ...
                <button
                  type="submit"
                  disabled={loading || (captcha.enabled && !signInCaptcha)}
                  className="flex w-full items-center justify-center gap-2 rounded-xl bg-brand-600 py-3.5 text-sm font-semibold text-white shadow-lg shadow-brand-200 transition hover:bg-brand-700 disabled:opacity-60">
                ...
                <p className="border-t border-slate-100 pt-4 text-center text-[10px] font-medium uppercase tracking-wider text-slate-400">
                  Authorized access only · © {new Date().getFullYear()} Spinzone Laundry
                </p>
              </form>
            ) : (
              <form onSubmit={handleRegister} className="space-y-4">
                ...
                <div>
                  ...
                  <select value={regRole} onChange={(e) => setRegRole(e.target.value as 'customer' | 'admin')}
                ...
                <button
                  type="submit"
                  disabled={loading || (captcha.enabled && !regCaptcha)}
